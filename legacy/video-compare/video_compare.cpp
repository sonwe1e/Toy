...
VideoCompare::VideoCompare(const VideoCompareConfig& config)
    : config_(config),
      ...
      frame_prefetch_count_(config.frame_prefetch_count),
  ...
  display_ = std::make_unique<Display>(config_.display_number, config_.display_mode, config_.verbose, config_.fit_window_to_usable_bounds, config_.high_dpi_allowed, config_.ui_scale, config_.aspect_lock_mode, config_.aspect_view_mode,
                                       config_.use_10_bpc, use_fast_input_alignment(config_), config_.bilinear_texture_filtering, config_.window_size, max_width_, max_height_, shortest_duration_, config_.wheel_sensitivity,
                                       config_.start_in_subtraction_mode, config_.start_in_fullscreen, config_.left.file_name, right_file_name);
...
void VideoCompare::compare() {
  try {
    ...
    int frame_offset = 0;
    int64_t newest_frame_index = -1;

    const AVRational frame_index_rate = demuxers_[LEFT]->guess_frame_rate();
    const bool has_frame_index_rate = frame_index_rate.num > 0 && frame_index_rate.den > 0;
    const auto estimate_frame_index = [&](const int64_t pts) -> int64_t {
      if (!has_frame_index_rate || pts <= 0) {
        return 0;
      }
      return av_rescale_q_rnd(pts, AVRational{1, AV_TIME_BASE}, av_inv_q(frame_index_rate), static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
    };
    const int64_t estimated_total_frames = has_frame_index_rate ? std::max<int64_t>(1, av_rescale_q(demuxers_[LEFT]->duration(), AVRational{1, AV_TIME_BASE}, av_inv_q(frame_index_rate))) : 0;
    ...
    const bool frame_navigation_smoke_test = env_flag_enabled("VIDEO_COMPARE_FRAME_NAV_SMOKE_TEST");
    const SDL_Keycode smoke_test_keys[] = {SDLK_a, SDLK_a, SDLK_d};
    const int smoke_test_expected_deltas[] = {-1, -2, -1};
    int smoke_test_stage = 0;
    int64_t smoke_test_base_frame_index = -1;
    bool smoke_test_step_injected = false;
    bool smoke_test_complete = false;
    ...
    for (uint64_t frame_number = 0;; ++frame_number) {
      ...
      // Release-package integration hook. It is completely dormant unless the
      // environment flag is explicitly set. At EOF, exercise the same SDL
      // KEYDOWN path as the A/A/D controls, then verify the selected decoded
      // frame index after every cache navigation step.
      if (frame_navigation_smoke_test && reached_end_of_stream && !smoke_test_complete && !smoke_test_step_injected) {
        if (smoke_test_base_frame_index < 0) {
          smoke_test_base_frame_index = newest_frame_index;
        }

        SDL_Event smoke_event{};
        smoke_event.type = SDL_KEYDOWN;
        smoke_event.key.type = SDL_KEYDOWN;
        smoke_event.key.timestamp = SDL_GetTicks();
        smoke_event.key.state = SDL_PRESSED;
        smoke_event.key.repeat = 0;
        smoke_event.key.keysym.sym = smoke_test_keys[smoke_test_stage];
        display_->handle_event(smoke_event);
        smoke_test_step_injected = true;
      }
      ...
      // A/D first navigate the decoded frame-pair cache. Only a cache miss asks
      // the decoder for more data (forward) or refills from an earlier GOP
      // (backward).
      if (frame_navigation_delta != 0) {
        const size_t common_cached_frames = std::min(left.frames_.size(), right_ptr->frames_.size());
        const FrameNavigationPlan navigation = plan_frame_navigation(frame_offset, common_cached_frames, frame_navigation_delta);
        frame_offset = navigation.frame_offset;
        forward_navigate_frames += navigation.decode_forward;

        if (navigation.seek_backward > 0) {
          seek_relative -= static_cast<float>(navigation.seek_backward) * (static_cast<float>(left_or_right_delta) * AV_TIME_TO_SEC);
        }
      ...
      if ((seek_relative != 0.0F) || (shift_right_frames != 0) || force_seek_current_position) {
        ...
        std::map<Side, int64_t> seek_target_pts;
        const auto position_to_pts = [](const SideState& side_state, const float position) {
          return static_cast<int64_t>(std::llround((position - side_state.start_time_) / AV_TIME_TO_SEC));
        };

        // the left video is the "master"
        const int64_t selected_left_pts = frame_offset >= 0 && frame_offset < static_cast<int>(left.frames_.size()) ? left.frames_[frame_offset]->pts : left.pts_;
        const float min_left_position = (left.first_pts_ > INT64_MIN) ? (left.first_pts_ * AV_TIME_TO_SEC + left.start_time_) : left.start_time_;
        const float left_position = selected_left_pts * AV_TIME_TO_SEC + left.start_time_;
        ...
        auto compute_right_position = [&](const SideState& right_state) -> float { return selected_left_pts * AV_TIME_TO_SEC + right_state.start_time_; };
        ...
        for (auto& pair : side_states) {
          ...
          if (side.is_right()) {
            ...
            seek_target_pts[side] = position_to_pts(right_state, next_right_position);
        ...
        seek_target_pts[LEFT] = position_to_pts(left, next_left_position);
        ...
        if (seek_failed) {
          ...
          seek_target_pts[LEFT] = selected_left_pts;
          ...
          for (auto& pair : side_states) {
            ...
            if (side.is_right()) {
              ...
              seek_target_pts[side] = selected_left_pts;
            }
          }
        }
        ...
        // av_seek_frame() lands on a nearby keyframe. Decode all streams in
        // round-robin order until each reaches the requested presentation
        // point; this avoids exposing the GOP's I-frame as the requested frame
        // and also prevents a bounded queue on another stream from deadlocking.
        std::map<Side, bool> seek_target_reached;
        for (const auto& pair : side_states) {
          seek_target_reached[pair.first] = false;
        }
        ...
        for (;;) {
          ...
          for (auto& pair : side_states) {
            ...
            if (seek_target_reached[side_state.side_]) {
              continue;
            }

            all_targets_reached = false;
            AVFrameUniquePtr candidate{nullptr, avframe_deleter};
            if (!converted_frame_queues_[side_state.side_]->pop(candidate)) {
              seek_target_reached[side_state.side_] = true;
              continue;
            }

            made_progress = true;
            const int64_t tolerance = std::max<int64_t>(1, side_state.delta_pts_ > 0 ? side_state.delta_pts_ / 4 : 1);
            if (candidate != nullptr && candidate->pts + tolerance >= seek_target_pts[side_state.side_]) {
              side_state.frame_ = std::move(candidate);
              seek_target_reached[side_state.side_] = true;
            }
          }

          if (all_targets_reached || !made_progress) {
            break;
          }

          all_targets_reached = true;
          for (const auto& pair : seek_target_reached) {
            all_targets_reached = all_targets_reached && pair.second;
          }
        ...
        auto reset_after_precise_seek = [&](SideState& side_state, int64_t* effective_time_shift = nullptr) {
        ...
        for (auto& pair : side_states) {
          ...
          if (side.is_right()) {
            ...
            reset_after_precise_seek(right_state, &right_state.effective_time_shift_);
          }
        }

        frame_offset = 0;
        newest_frame_index = left.frame_ != nullptr ? estimate_frame_index(left.frame_->pts) : -1;
      ...
      const bool background_prefetch =
          !display_->get_play() &&
          display_->get_buffer_play_loop_mode() == Display::Loop::Off &&
          forward_navigate_frames == 0 &&
          !reached_end_of_stream &&
          frame_prefetch_needed(frame_offset, frame_prefetch_count_) > 0;
      const bool paused_forward_step = !display_->get_play() && (forward_navigate_frames > 0 || background_prefetch);
      skip_update = skip_update || ((timer_->us_until_target() - refresh_time_deque.average()) > 0 && !paused_forward_step);
      const bool fetch_next_frame = display_->get_play() || (forward_navigate_frames > 0) || background_prefetch;
      ...
      // Frame-index mode pairs the Nth decoded frame from every input. Timestamp
      // mode retains the legacy PTS-based drop/adjust behaviour.
      if (sync_mode_ == SyncMode::Timestamp) {
        for (auto& pair : side_states) {
          if (pair.first.is_right()) {
            SideState& right_state = pair.second;
      ...
      if (store_frames) {
        newest_frame_index = newest_frame_index < 0 ? estimate_frame_index(left.frame_->pts) : newest_frame_index + 1;
      }
      ...
      // Prefetching must not visibly advance the selected frame. A newly
      // decoded frame is inserted at offset zero, so move the selection one
      // slot toward the older end to keep the same logical frame on screen.
      if (store_frames && background_prefetch && !display_->get_play() && forward_navigate_frames == 0) {
        frame_offset++;
      }
      ...
      reached_end_of_stream = reached_end_of_stream || end_of_file;
      if (store_frames) {
        reached_end_of_stream = false;
      }
      ...
      if (smoke_test_step_injected) {
        const int64_t displayed_frame_index = std::max<int64_t>(0, newest_frame_index - frame_offset);
        const int64_t expected_frame_index = smoke_test_base_frame_index + smoke_test_expected_deltas[smoke_test_stage];
        const char key = smoke_test_keys[smoke_test_stage] == SDLK_a ? 'A' : 'D';

        std::cerr << "[frame-nav-smoke] " << key << " -> frame " << (displayed_frame_index + 1)
                  << " (expected " << (expected_frame_index + 1) << ")" << std::endl;

        if (displayed_frame_index != expected_frame_index) {
          throw std::runtime_error("Frame navigation smoke test failed");
        }

        smoke_test_step_injected = false;
        ++smoke_test_stage;
        if (smoke_test_stage == 3) {
          smoke_test_complete = true;
          std::cerr << "[frame-nav-smoke] PASS: A/A/D selected exact synchronized cached frames" << std::endl;
        }
      }
      ...
      if (frame_offset >= 0 && !left.frames_.empty() && !right_ptr->frames_.empty()) {
        const bool is_playback_in_sync = sync_mode_ == SyncMode::FrameIndex || is_in_sync(left.pts_, right_ptr->pts_, left.delta_pts_, right_ptr->delta_pts_);
        ...
        if (!skip_refresh) {
          ...
          if (frame_number >= next_refresh_frame_number) {
            const int64_t displayed_frame_index = std::max<int64_t>(0, newest_frame_index - frame_offset);
            const int cached_behind = std::max(0, last_common_frame_index - frame_offset);
            const int cached_ahead = std::max(0, frame_offset);
            const std::string current_total_browsable =
                estimated_total_frames > 0
                    ? string_sprintf("Frame %lld/%lld  cache -%d/+%d", static_cast<long long>(displayed_frame_index + 1), static_cast<long long>(estimated_total_frames), cached_behind, cached_ahead)
                    : string_sprintf("Frame %lld  cache -%d/+%d", static_cast<long long>(displayed_frame_index + 1), cached_behind, cached_ahead);
