...
void Display::handle_event(const SDL_Event& event) {
  ...
  auto queue_frame_navigation = [&](const int direction) {
    // Frame navigation is an explicit inspection action. Freeze regular
    // playback first so the requested cached frame is not immediately
    // replaced by the next playback tick.
    play_ = false;
    buffer_play_loop_mode_ = Loop::Off;
    tick_playback_ = true;
    frame_navigation_delta_ += direction;
  };
  ...
  switch (event_.type) {
    ...
    case SDL_MOUSEBUTTONUP:
      ...
      break;
    case SDL_TEXTINPUT: {
      // SDL normally emits KEYDOWN followed by TEXTINPUT for a physical
      // keyboard. Touch keyboards and accessibility input may emit only
      // TEXTINPUT, so accept a single ASCII A/D here while suppressing the
      // duplicate text event paired with a recent keydown.
      const uint32_t timestamp_delta = event_.text.timestamp - last_frame_navigation_keydown_timestamp_;
      const bool follows_navigation_keydown =
          last_frame_navigation_keydown_timestamp_ != 0 && timestamp_delta <= 100;
      const char key = event_.text.text[0];
      if (!follows_navigation_keydown && event_.text.text[1] == '\0') {
        if (key == 'a' || key == 'A') {
          queue_frame_navigation(-1);
        } else if (key == 'd' || key == 'D') {
          queue_frame_navigation(1);
        }
      }
    ...
    case SDL_KEYDOWN: {
      ...
      switch (keycode) {
        ...
        case SDLK_a:
          last_frame_navigation_keydown_timestamp_ = event_.key.timestamp;
          queue_frame_navigation(-1);
          break;
        case SDLK_d:
          last_frame_navigation_keydown_timestamp_ = event_.key.timestamp;
