  ...
  try {
    argagg::parser argparser{
        {{"help", {"-h", "--help"}, "print help and exit", 0},
         ...
         {"frame-buffer-size", {"-f", "--frame-buffer-size"}, "decoded frame-pair cache size, default is 11 (current frame +/- 5)", 1},
         {"frame-prefetch", {"--frame-prefetch"}, "number of frames to decode ahead while paused, default is 5", 1},
         {"sync-mode", {"--sync-mode"}, "pair videos by decoded frame index (frame, default) or presentation timestamp (time)", 1},
         ...
         {"left-decoder", {"--left-decoder"}, "left FFmpeg video decoder name, specified as [type?][:options?] (e.g. ':strict=-2,trust_dec_pts=1' or 'h264:trust_dec_pts=1,rewrite_duration=1')", 1},
         {"right-decoder", {"--right-decoder"}, "right FFmpeg video decoder name, specified as [type?][:options?] (e.g. ':rewrite_duration=1')", 1},
    ...
    if (args["show-controls"]) {
      ...
      if (args["frame-prefetch"]) {
        const std::string frame_prefetch_arg = args["frame-prefetch"];
        if (!std::regex_match(frame_prefetch_arg, UNSIGNED_INTEGER_RE)) {
          throw std::logic_error{"Cannot parse frame prefetch count (required format: [number], e.g. 0, 5 or 10)"};
        }

        config.frame_prefetch_count = std::stoi(frame_prefetch_arg);
      }
      if (args["sync-mode"]) {
        const std::string sync_mode_arg = args["sync-mode"];
        if (sync_mode_arg == "frame") {
          config.sync_mode = SyncMode::FrameIndex;
        } else if (sync_mode_arg == "time") {
          config.sync_mode = SyncMode::Timestamp;
        } else {
          throw std::logic_error{"Cannot parse sync mode argument (valid options: frame, time)"};
        }
      }
      if (!args["frame-prefetch"]) {
        config.frame_prefetch_count = std::min(config.frame_prefetch_count, (config.frame_buffer_size - 1) / 2);
      } else if (config.frame_buffer_size < config.frame_prefetch_count + 1) {
        throw std::logic_error{"Frame buffer size must be greater than the frame prefetch count"};
      }
      if (args["time-shift"]) {
