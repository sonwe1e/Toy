...
## Frame-indexed comparison

The default synchronization model is frame-indexed: the decoder takes one decoded frame from each input and stores
the two frames as a pair. This is intended for VFI, super-resolution, enhancement, and compression review, where
`frame N` in the left input should be compared with `frame N` in the right input.

`A` and `D` always pause playback and show the previous or next frame. While paused, the application decodes five
frames ahead in the background and keeps a bounded 11-pair sliding cache by default. Cached steps are immediate. Crossing a cache
boundary causes FFmpeg to seek to an earlier keyframe and decode forward to the requested presentation point; the
keyframe itself is not exposed as the requested adjacent frame.

The HUD reports the logical frame number and the number of cached pairs behind/ahead of it. The total frame count is
estimated from duration and the detected frame rate when the container does not provide an exact count.

Use these options to tune or disable the new behavior:

    video-compare --frame-buffer-size 21 --frame-prefetch 10 video1.mp4 video2.mp4
    video-compare --sync-mode time video1.mp4 video2.mp4

`--sync-mode time` restores the legacy presentation-timestamp synchronization for sources whose frame rates or
temporal structures intentionally differ. The cache stays bounded in both modes; the application never decodes an
...
- `A`: Pause and show the previous frame (refills the cache when needed)
- `D`: Pause and show the next frame (uses prefetched frames when available)
...
- `Shift+D`: Next frame (alias for `D`)
- `Shift+A`: Previous frame (alias for `A`)
...
Run the frame-navigation state-machine test without launching the SDL UI:
